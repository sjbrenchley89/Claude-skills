...
### Model Descriptions
- **Claude Opus 4.8** — The most capable Claude model to date — highly autonomous, strong on long-horizon agentic work, knowledge work, vision, and memory. Adaptive thinking only. 1M context window at standard API pricing.
- **Claude Opus 4.7** — Previous-generation Opus. Highly autonomous, strong on agentic work. Adaptive thinking only; sampling parameters and `budget_tokens` are removed. 1M context window — see `shared/model-migration.md` → Migrating to Opus 4.7 for breaking changes.
